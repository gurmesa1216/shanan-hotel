...
export default function MenuManager({ refreshDishes, categories = [] }) {
  ...
  // OPEN IMAGE SELECTOR (Local File or URL)
  const triggerImageChange = (dish) => {
    setSelectedDishForImage(dish);
    const choice = window.confirm("Click OK to upload a photo from Local Storage (Device), or CANCEL to enter an Image URL.");
    ...
    if (choice) {
      ...
      if (imageUrl) {
        updateDishImage(dish.id, imageUrl);
      }
    }
  };
  ...
  const handleFileChange = (e) => {
    ...
    reader.onloadend = () => {
      ...
      updateDishImage(selectedDishForImage.id, base64Image);
      // Reset input value to allow selecting same image again if needed
      e.target.value = "";
    };
    reader.readAsDataURL(file);
  };

  // UPDATE DISH IMAGE API CALL
  const updateDishImage = async (dishId, imageUrl) => {
  ...
  return (
    <div className="menu-manager">
      {/* Hidden File Input for Device Local Storage Selection */}
      <input 
        type="file" 
        ref={fileInputRef} 
        style={{ display: "none" }} 
        ...
        {dishes.map((dish) => (
          <div className="menu-card" key={dish.id}>
            <div className="menu-card-image-wrapper">
              ...
              <button className="delete-btn" onClick={() => handleDelete(dish.id)} aria-label="Delete Dish">
                <TrashIcon />
                <span>Delete</span>
              </button>

              <span className={`status-badge ${dish.available ? 'status-badge--active' : 'status-badge--hidden'}`}>
            ...
            <div className="menu-card-body">
              <h3>{dish.name}</h3>
              <p className="menu-card-price">{dish.price} <span className="currency">ETB</span></p>
              ...
              <div className="menu-card-actions">
                ...
                <button className="action-btn" onClick={() => triggerImageChange(dish)}>
                  <ImageIcon />
                  <span>Change Image</span>
                </button>

                <button 
                  className={`action-btn toggle-btn ${dish.available ? 'toggle-btn--hide' : 'toggle-btn--show'}`} 
